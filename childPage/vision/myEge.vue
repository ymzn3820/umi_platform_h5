<template>
	<view>
		...
		<!-- <view style="text-align: center;color: #888F9B;font-size: 24upx;margin: 30upx 0;">滑动查看对比</view> -->
		<!-- <slider class="slider" v-model="blur" min="0" max="10" @change="handleSliderChange" block-color='#1F64FF'
			block-size='16'></slider> -->
		<view class="bottom">
		...
		<view class="footer">
			<view class="text">本次生成将消耗8.4w算力</view>
	...
	export default {
		data() {
			return {
				...
				default: global.baseImg + '/xcx/com/message_center/%E5%A4%A7%E5%9B%BE.png',
				list: [{
					...
					{
						src: global.baseImg + '/xcx/com/message_center/%E5%8F%98%E8%80%81%E7%82%B9.png',
						name: '变老点'
					}
				],
		...
		methods: {
					...
					if (res.code == 20000) {
						...
						arr.forEach((item, index) => {
							if (index == arr.length - 2) {
								if (item.change_degree == 70) {
									this.active = 2
								}
								if (item.change_degree == 5) {
									this.active = 1
								}
			...
			historyText() {
				uni.navigateTo({
					url: '/childPage/vision/history_text?chat_type=16'
				})
			},
			previewImage() {
				uni.previewImage({
