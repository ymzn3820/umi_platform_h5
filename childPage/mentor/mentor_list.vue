<template>
	<view>
			...
			<scroll-view class="list_ul" scroll-y="true" v-if="infoList.length > 0">
				<view class="cont_list" v-for="(item, index) in infoList" :key="index">
					<view class="list_li">
						...
						<view class="li_edit">
							<view class="edit_l"
								@click="mentorChat(item.avatar, item.greeting,item.me_id,item.tutor_id)">对话</view>
							<view class="edit_r" @click.stop="selectRole(item, 1)">选择导师</view>
						</view>
		...
		<view v-if="me_type == 2">
			<scroll-view class="list_ul" scroll-y="true" v-if="myList.length > 0">
				<view class="ul_li" v-for="(res, tip) in myList" :key="tip">
					...
					<view class="cont_list" v-for="(item, index) in res.tutor" :key="index"
						v-if="JSON.stringify(item) != '{}'">
						<view class="list_li" @click="toDetail(item.tutor_id)">
							<view class="li_lines">
								<view class="line_l">
									<image class="li_img" mode="aspectFit" :src="getImgUrl(item.avatar)"></image>
									<text class="li_title">{{ item.character_name }}</text>
								</view>
								<view @click.stop="delMentor(item.tutor_id, item.me_id)"><u-icon name="trash"
										size="20"></u-icon></view>
							</view>
							<view class="li_cont">{{ item.introduction }}</view>
							...
							<view class="li_edit">
								<view class="edit_l"
									@click.stop="mentorChat(item.avatar, item.greeting,item.me_id,item.tutor_id)">对话
								</view>
								<view class="edit_r" @click.stop="selectRole(item, 0)">选择导师</view>
							</view>
		...
		<view class="wz_tip" @click="toCreate">
...
<script>
	...
	export default {
		...
		methods: {
			...
			toCreate() {
				uni.navigateTo({
					url: '/childPage/mentor/add_list?me_id=' + this.me_id + '&sort=' + this.sort + '&edit_id=' +
						this.tutor_id,
				})
			},
			...
			selectRole(item, tip) {
				...
				uni.showModal({
					...
					success(res) {
						if (res.confirm) {
							if (this.tutor_id != '') {
								that.editRole(item, tip)
							} else {
								that.addRole(item, tip, '')
							}
			...
			editRole(item, tip) {
				...
				editTutor(data).then(res => {
					if (res.code == 20000) {
						this.addRole(item, tip, this.tutor_id)
					} else {
						this.$api.msg(res.msg);
						uni.hideLoading()
					}
