<template>
	<view>
		...
		<view class="chat_cont">
			<scroll-view class="list_ul" scroll-y="true" v-if="chatList.length > 0" :style="[{marginTop:'220rpx'}]">
					...
					<view class="li_style">
						...
						<view class="li_right">
							<view class="li_top">
								...
								<text class="top_r"
									v-if="item.chat_type == 4 && item.model === 'completions_pro'">文心一言-4</text>
								<text class="top_r"
									v-if="item.chat_type == 4 && item.model === 'completions'">文心一言</text>
								<text class="top_r"
									v-if="item.chat_type == 4 && item.model === 'eb-instant' ">文心一言-Turbo</text>
								...
								<text class="top_r"
									v-if="item.chat_type == 10 && item.model == 'qwen-plus' ">通义千问-plus</text>
								<text class="top_r"
									v-if="item.chat_type == 10 && item.model == 'qwen-turbo' ">通义千问-turbo</text>
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							if (that.active == 4 || that.active == 5 || that.active == 7 || that.active == 8 ||
								that.active == 10 || that.active == 12) {
								uni.navigateTo({
									url: '/childCont/repository/agent?item='+that.item
								})
							} else {
								uni.switchTab({
									url: '/pages/draw/draw'
