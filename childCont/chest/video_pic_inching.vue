<template>
	<view style="background-color: #FEFEFE;height: 100vh;">
		<view class="bigBox">
			<image v-if="result_image==''" class="bigImg" mode="aspectFit" :src="src"></image>

			<video v-else class="bigVideo" :src="result_image" lautoplay loop muted object-fit='contain'></video>
			<image v-if="result_image==''" class="upImg" @click="previewImage" :src="icon_img" mode="aspectFit"></image>
	...
	export default {
		data() {
			return {
				...
				active: 0,
				blur: 0,
				src: global.baseImg + '/' + 'xcx/com/message_center/bbxbigpic.png', //大图
				...
				list: [{
					...
					{
						src: global.baseImg + '/' + 'xcx/com/message_center/bbx.png',
						name: '开启微动'
					}
				],
			};
		...
		methods: {
			...
			async mergeChange(index) {
				...
				if (index !== 0) {
					...
					if (res.code == 20000) {
						this.result_image=global.baseImg + '/' + res.data.results[0].result_image
						// this.result_image = imgUrl.replace('.avi','.mp4')
			...
			historyText() {
				uni.navigateTo({
					url: '/childCont/chest/history_chest?chat_type=34'
				})
			},
			toSlove() {
				let that = this;
