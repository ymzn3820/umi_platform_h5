<template>
	<view style="background-color: #FEFEFE;height: 100vh;">
		...
		<view class="footer">
			<view class="text">本次生成将消耗60w算力/次</view>
		...
		<view class="der_setting" v-if="setList!=''">
			...
			<view class="item" v-for="item,index in Object.entries(setList)" :key="index">
				<view class="item_l">{{item[0]}}</view>
				<view class="item_r" v-if="item[1] !=''">{{item[1]}}</view>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				active: 0,
				blur: 0,
				src: global.baseImg + '/xcx/com/message_center/pfbjc.png', //大图
			...
			async mergeChange(index) {
				...
				if (index !== 0) {
					...
					if (res.code == 20000) {
						...
						this.setList = res.data.results[0].result_list
