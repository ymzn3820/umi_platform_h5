<template>
	<view style="background-color: #FEFEFE;height: 100vh;"
		:style="[{backgroundColor:(proShow?'rgba(0, 0, 0, 0.60)':'#FEFEFE')}]" @click="proShow=false">
		<view class="bigBox">
		...
		<view class="footer">
			...
			<view v-if="proShow==false" class="record" @click="historyText">历史记录</view>
			<button v-if="proShow==false" style="background-color: #1F64FF;color: #fff;" class="btn" size="default"
				type="default" @click="toSlove">上传图片</button>
			<button v-else style="background-color: #1F64FF;color: #fff;" class="btn" size="default" type="default"
				@click="proShow=false">返回</button>
		</view>

		<!-- 商品框 -->
		<view class="product" v-if="proShow">
			<view class="pro_head">
				<u-icon name="search" color="#000" size="24"></u-icon>
				<view class="pro_ht">为你找到以下相识商品</view>
				<u-icon name="close" color="#000" size="24" @click="proShow=false"></u-icon>
			</view>
			<view class="pro_main">
				<view class="pro_mi">
					<image :src="src" class="pro_img"></image>
					<view class="pro_mit">卡塔尔世界杯阿根廷三星球卡塔尔世界杯阿根廷三星球</view>
				</view>
			</view>
		</view>
	</view>
